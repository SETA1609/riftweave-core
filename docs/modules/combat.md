...
- **Weapon on-hit effects**: The `weapon` object supports `on_hit_effects` — an array of `appliedEffect` shapes bridging weapons to the shared effect registry. Additionally, crafted items have their own `on_hit_effects` that can extend or override the base weapon's effects. Both sources trigger condition application and dispel checks. See `docs/combat/integration.md` and `docs/modules/conditions.md` for details.
